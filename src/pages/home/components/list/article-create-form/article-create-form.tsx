...
const ArticleCreateForm: React.FC<ArticleCreateFormProps> = ({ errorMsg, onArticleCreate }) => {
 ...
 const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onArticleCreate({ titleEn, descriptionEn, titleKa, descriptionKa, imageSrc });
    setTitleEn('');
    setDescriptionEn('');
    setTitleKa('');
    setDescriptionKa('');
  ...
  return (
    <form onSubmit={handleSubmit} style={{
      ...
      <div>
        ...
        <input type="text" value={descriptionEn} onChange={(e) => setDescriptionEn(e.target.value)} />
      </div>
      <div>
        <label>Georgian Title:</label>
        <input type="text" value={titleKa} onChange={(e) => setTitleKa(e.target.value)} />
      </div>
      <div>
        <label>Georgian Description:</label>
        <input type="text" value={descriptionKa} onChange={(e) => setDescriptionKa(e.target.value)} />
      </div>
      <div>
     ...
     <input
      ...
      <button type="submit" style={{ marginLeft: 0,width: 100, margin: "auto", }} >Create</button>
    </form>
  );
};
